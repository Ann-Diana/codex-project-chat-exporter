...
The productive consumer is the export-local deduplicated asset store. It independently hashes the same decoded blocks, validates bounded headers, publishes one file per decoded SHA-256, and records every committed use. This independent hash/byte comparison prevents the sink from trusting a filename or descriptor alone. The complete contract is documented in [asset-store.md](asset-store.md).
