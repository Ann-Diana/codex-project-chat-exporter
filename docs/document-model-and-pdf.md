...
The renderer uses only the bundled Noto Sans, Noto Sans Mono, Noto Sans Symbols, Noto Sans Symbols 2, and monochrome Noto Emoji faces described in `fonts/README.md`. Font bytes are hashed before parsing. Text is segmented into Unicode graphemes; missing primary glyphs use the two symbol faces, the emoji face, and then the monospace face in a fixed order. An explicit emoji-presentation selector instead prefers the emoji face before the symbol faces so text and emoji presentations cannot collide in one PDF subset's ToUnicode map. Variation selectors and joiners stay attached to their grapheme instead of being treated as standalone printable glyphs. Arrows, check marks, warning signs, emoji, plus/minus, and comparison signs remain selectable as their original Unicode characters. Each mixed-font run derives its baseline, pagination, and line height from the primary face's ascender/descender metrics instead of applying character-specific offsets.

If no bundled face can represent a valid grapheme, PDF alone renders a deterministic visible marker such as `[unsupported glyph U+10FFFF]`; it does not mutate the shared document model, Markdown, HTML, DOCX, Raw JSONL, or source snapshots. A malformed unpaired UTF-16 surrogate remains a controlled PDF error rather than being mislabeled as a valid Unicode glyph. No operating-system font, remote lookup, image substitution, or runtime installation is used.
