...
Attachment occurrences use the same classified record selection as text. Tool-only images require `includeTools`; verified source mirrors render once. Readable omits every `replacement_history` occurrence. Complete renders unmatched history images in a labelled `Additional stored context` section. Equal bytes in independent genuine turns remain separate visible occurrences.
...
The renderer uses only the bundled Noto Sans, Noto Sans Mono, Noto Sans Symbols, Noto Sans Symbols 2, and Noto Emoji 3.002 variable monochrome outline faces described in `fonts/README.md`. Font bytes are hashed before parsing. Text is segmented into Unicode graphemes; missing primary glyphs use the two symbol faces, the emoji face, and then the monospace face in a fixed order. An explicit emoji-presentation selector instead prefers the emoji face before the symbol faces so text and emoji presentations cannot collide in one PDF subset's ToUnicode map. Multi-codepoint graphemes must retain their complete ordered code-point mapping after shaping. Variation selectors and joiners therefore remain attached to one supported grapheme, or the complete grapheme receives one visible marker; neither can silently disappear or render as an independent false character. Arrows, check marks, warning signs, emoji, plus/minus, and comparison signs remain selectable as their original Unicode characters. Each mixed-font run derives its baseline, pagination, and line height from the primary face's ascender/descender metrics instead of applying character-specific offsets.

If no bundled face can represent a valid grapheme, PDF alone renders a deterministic visible marker such as `[unsupported glyph U+10FFFF]` or `[unsupported glyph U+1F604 U+200D U+1F604]`; the multi-codepoint form lists every code point in source order. It does not mutate the shared document model, Markdown, HTML, DOCX, Raw JSONL, or source snapshots. A malformed unpaired UTF-16 surrogate remains a controlled PDF error rather than being mislabeled as a valid Unicode glyph. No operating-system font, remote lookup, image substitution, or runtime installation is used.
