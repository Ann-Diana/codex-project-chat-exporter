...
Invalid JSON, truncated records, invalid UTF-8, invalid embedded Base64, source failures, and projection failures abort the productive reader. A record is invisible until `record_end`; the asset sink writes only to run-owned staging and publishes after record commit. Raw JSONL remains canonical and byte-identical; the deduplicated asset store, Markdown, and HTML remain derived views. DOCX, PDF, and VSIX packaging are separate follow-up work.
