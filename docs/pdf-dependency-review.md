...
Noto Sans 2.015 provides proportional regular, bold, and italic text; Noto Sans Mono 2.014 provides code text and mathematical comparison fallbacks. Noto Sans Symbols 2.003 supplies arrows, while the separately released Noto Sans Symbols 2 2.008 supplies check marks and warning signs absent from the other bundled faces. The pinned Noto Emoji 3.002 variable monochrome outline TTF from Google Fonts commit `ade3d1533e06b2b1462ffcde8e08b129627ca360` supplies the confirmed readable U+1F604 outline and supported complex emoji. Its unchanged 1,982,596 bytes have SHA-256 `de6c18832938afc99caf132b39d6a30a19bac7f2e812e28db2535b4608d27551`; PDFKit embeds the default 400-weight instance as selectable subset text with ToUnicode mappings.

Every emitted grapheme is checked against the selected primary face followed by the deterministic symbol/emoji/monospace fallback chain; an explicit emoji-presentation selector prefers the emoji face before the symbol faces to keep distinct ToUnicode mappings. Multi-codepoint shaping must preserve the complete ordered Unicode mapping, so supported variation-selector and ZWJ graphemes remain intact while an unsupported complex grapheme receives one marker containing every code point. Invalid unpaired UTF-16 remains an error without source text. Font-specific ascender, descender, and line-gap metrics are normalized to the primary face for every fallback run, preserving a shared baseline and the primary line-height/wrapping contract.

The Noto Sans, Symbols, and Emoji files are official assets under SIL OFL-1.1. Exact versions, source archives or commits, and SHA-256 values are recorded in `fonts/README.md`; `fonts/OFL.txt`, `fonts/OFL-SYMBOLS.txt`, and `fonts/OFL-EMOJI.txt` preserve the distinct upstream notices. All font and license files are included in the VSIX integrity manifest and copied by the regular builder.
