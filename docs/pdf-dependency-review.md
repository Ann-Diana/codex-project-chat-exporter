...
Noto Sans 2.015 provides proportional regular, bold, and italic text; Noto Sans Mono 2.014 provides code text. A gate test showed that Noto Sans alone does not cover U+2192, so Noto Sans Symbols 2.003 is bundled as an explicit fallback. The union covers the required umlauts, typographic quotation marks, arrows, and common technical punctuation. Every emitted code point is checked against the selected primary face and symbol fallback; an uncovered point aborts with only the session ID and `U+NNNN`, never the source text.

The font files are official release assets under SIL OFL-1.1. Their exact versions, source archives, and SHA-256 values are recorded in `fonts/README.md`, included in the VSIX integrity manifest, and copied by the regular builder.
