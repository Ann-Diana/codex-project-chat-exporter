...
The root `manifest.json` points to `assets/manifest.json`, records the effective `include_tools` value, and retains `asset_occurrences`, `unique_assets`, `unique_asset_bytes`, and `deduplicated_asset_bytes_saved`. Raw JSONL remains the canonical import source; assets and reading views are derived outputs.
...
`replacement_history` is a stored context snapshot, not an ordinary chat turn. An exact semantic message copy is paired by role, ordered content identity, attachment source identity, and unambiguous occurrence order—not by asset hash alone. Multiple identical visible messages are paired only when the snapshot contains the complete ordered occurrence set; an incomplete ambiguous copy is retained as stored context. A verified copy of an already visible message is a non-rendered mirror. An image with no matching visible original appears once in a labelled `Additional stored context` section; later exact copies in subsequent snapshots point to that canonical stored-context occurrence.
