...
In **Codex Export: Export…**, choose **Choose recorded project path…**, the export profile and optional document format. The picker displays each distinct recorded `cwd`, retained session count, total source bytes and latest session timestamp. It uses the existing discovery/routing pass and active-over-archive duplicate-session preference; duplicate copies are not counted twice. Sessions whose working directory changes use the same final routing cwd as normal selection.
...
There is no implicit all-session fallback or additional complete JSONL pass for the dialog. Source stability checks still apply after interactive selection.
