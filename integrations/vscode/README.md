...
- **Current Workspace** discovers detected active and archived sessions through a bounded first-record metadata stream and exports only sessions with the same absolute path identity as the currently opened local workspace. Windows representation differences such as drive-letter case, slash direction, a trailing separator or an equivalent local `\\?\` prefix are normalized without fuzzy matching. Discovery is cancellable and does not read later conversation records to determine paths.
