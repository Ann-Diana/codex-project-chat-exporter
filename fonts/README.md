...
| `NotoEmoji-Variable.ttf` | Noto Emoji 3.002 variable monochrome outline TTF at immutable Google Fonts commit `ade3d1533e06b2b1462ffcde8e08b129627ca360` | `de6c18832938afc99caf132b39d6a30a19bac7f2e812e28db2535b4608d27551` |
...
The Latin/Greek/Cyrillic, symbol, and emoji files are licensed under the SIL Open Font License 1.1. `OFL.txt` preserves the Latin/Greek/Cyrillic font project's copyright notice, `OFL-SYMBOLS.txt` preserves the separate Noto Symbols project's notice for both symbol faces, and `OFL-EMOJI.txt` is the unmodified Noto Emoji license from the pinned Google Fonts commit. The unchanged emoji source file is `https://github.com/google/fonts/blob/ade3d1533e06b2b1462ffcde8e08b129627ca360/ofl/notoemoji/NotoEmoji%5Bwght%5D.ttf`.

The renderer verifies every font hash before use. Regular, bold, italic, monospace, two complementary symbol faces, and the variable monochrome emoji outline face at its default 400 weight are embedded as subsets only when needed. Multi-codepoint graphemes must preserve their complete ordered code-point mapping during shaping; otherwise the complete grapheme receives one deterministic visible PDF-only marker. Source data and the shared document model are unchanged.
