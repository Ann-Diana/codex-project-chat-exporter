...
All files are licensed under the SIL Open Font License 1.1. `OFL.txt` preserves the Latin/Greek/Cyrillic font project's copyright notice, while `OFL-SYMBOLS.txt` preserves the separate Noto Symbols project's copyright notice for both symbol faces. The renderer verifies every font hash before use. The regular, bold, italic, monospace, and two complementary symbol faces are embedded as subsets in each PDF as needed.
