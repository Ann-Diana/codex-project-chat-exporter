...
- project discovery streams only bounded first-record `session_meta` or `turn_context` metadata and never scans later JSONL conversation records to infer a path; recorded-project recovery uses only an explicitly selected exact cwd from that inventory, with adapter confirmation for a different workspace path; stored cwd values never authorize filesystem collection or automatic fallback;
