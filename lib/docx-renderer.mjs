...
function renderInlines(inlines) {
  return inlines.map((inline) => {
    if (inline.kind === "link") {
      if (inline.blocked) return new TextRun({ text: `${inline.label} [blocked ${inline.reason}]`, color: "9C2F2F" });
      return new ExternalHyperlink({
        link: inline.target,
        children: [new TextRun({ text: `${inline.label} (${inline.target})`, color: "2E74B5", underline: {} })],
      });
    }
...
async function canonicalizeHyperlinkRelationshipIds(zip) {
  ...
  return new Map([
    [DOCUMENT_RELATIONSHIPS_PART, Buffer.from(js2xml(relationshipDocument, { compact: false, spaces: 0 }), "utf8")],
    ["word/document.xml", Buffer.from(js2xml(document, { compact: false, spaces: 0 }), "utf8")],
  ]);
}
