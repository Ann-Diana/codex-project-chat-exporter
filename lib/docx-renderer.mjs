...
export async function buildDeterministicDocx(options = {}) {
  throwIfAborted(options.abortSignal, "DOCX rendering");
  ...
  const checkpoint = createAbortCheckpoint(options.abortSignal, "DOCX block processing", 32);
  ...
  for (const message of messages) {
    ...
    for (const block of message.blocks) {
      await checkpoint();
      children.push(...renderBlock(block, numbering));
    }
  ...
  let packed;
  try {
    throwIfAborted(options.abortSignal, "DOCX packaging");
  ...
  return normalizeAndValidateDocx(packed, { mediaReplacements, abortSignal: options.abortSignal });
}
...
export async function normalizeAndValidateDocx(buffer, options = {}) {
  throwIfAborted(options.abortSignal, "DOCX validation");
  let input;
  try {
  ...
  for (const entry of entries) {
    throwIfAborted(options.abortSignal, "DOCX normalization");
    let data;
...
export async function validateCanonicalDocx(buffer, options = {}) {
  throwIfAborted(options.abortSignal, "DOCX validation");
  let zip;
  try {
    ...
    return block.items.map((item) => {
      ...
      if (!references.has(sequence)) {
        ...
        numbering.push({ reference, levels: Array.from({ length: 9 }, (_, level) => ({
          ...
          text: ordered ? `%${level + 1}.` : (item.marker || "•"), alignment: AlignmentType.LEFT,
          style: { paragraph: { indent: { left: (block.announced ? 480 : 240) + level * 360, hanging: 240 } } },
        })) });
      }
...
function renderInlines(inlines) {
  return inlines.flatMap((inline) => {
    ...
    const lines = String(inline.text || "").split("\n");
    return lines.map((line, index) => new TextRun(index === 0
      ? ooxmlText(line)
      : { text: ooxmlText(line), break: 1 }));
  });
}
