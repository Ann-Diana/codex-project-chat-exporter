...
function renderInlines(inlines) {
  return inlines.map((inline) => {
    if (inline.kind === "link") {
      if (inline.blocked) return new TextRun({ text: `${inline.label} ${blockedLinkLabel(inline.reason)}`, color: "9C2F2F" });
...
function validateXmlPart(name, xml) {
  const validateCharacters = (value) => { for (const character of String(value)) {
    const cp = character.codePointAt(0);
    if (!(cp === 9 || cp === 10 || cp === 13 || (cp >= 0x20 && cp <= 0xd7ff) || (cp >= 0xe000 && cp <= 0xfffd) || (cp >= 0x10000 && cp <= 0x10ffff))) {
      throw new DocxExportError("DOCX_XML_INVALID", `Invalid XML 1.0 character in ${name}`);
    }
  } };
