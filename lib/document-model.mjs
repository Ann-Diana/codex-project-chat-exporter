...
import { formatModelHistory, normalizeModelHistory } from "./model-history.mjs";
...
export function createSessionDocumentHeader(meta = {}) {
  throwIfAborted(meta.abortSignal, "document model construction");
  const sessionId = requiredText(meta.id || meta.sessionId, "session ID");
  const models = normalizeModelHistory(meta.models || (meta.model ? [meta.model] : []));
  const modelSummary = formatModelHistory(models);
...
export function createDocumentMessage(options = {}) {
  throwIfAborted(options.abortSignal, "document model construction");
  ...
  const blocks = parseDocumentBlocks(String(options.text || ""), origin, options.abortSignal);
...
export function parseDocumentBlocks(text, origin, abortSignal = undefined) {
  throwIfAborted(abortSignal, "document model construction");
  ...
  const pushParagraph = () => {
    ...
    blocks.push(createTextBlock(DOCUMENT_BLOCK_KIND.PARAGRAPH, value, origin, blocks.length + 1, abortSignal));
    paragraph = [];
  };
  const pushList = () => {
    if (!list) return;
    ...
    blocks.push(Object.freeze({
      ...
      items: Object.freeze(list.items.map((item, index) => Object.freeze({
        ...
        marker: item.marker,
        announced: list.announced,
        inlines: parseInlineContent(item.text, origin, blockOrdinal, abortSignal),
      }))),
    }));
    list = null;
  };
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      blocks.push(Object.freeze({
        ...
        origin: blockOrigin(origin, blockOrdinal),
        level: heading.level,
        inlines: parseInlineContent(heading.text, origin, blockOrdinal, abortSignal),
      }));
      continue;
    }
    ...
    if (marker) {
      ...
      if (!list) list = { ordered: marker.ordered, items: [], stack: [], nextSequence: 0, announced: isAnnouncedList(blocks.at(-1)) };
      ...
      if (list.stack.length && marker.indent === list.stack.at(-1).indent
        && (marker.ordered !== list.stack.at(-1).ordered || marker.marker !== list.stack.at(-1).marker)) list.stack.pop();
      if (!list.stack.length || marker.indent > list.stack.at(-1).indent) {
        list.stack.push({ indent: marker.indent, ordered: marker.ordered, start: marker.start, marker: marker.marker, sequence: list.nextSequence++ });
      }
...
function createTextBlock(kind, text, origin, blockOrdinal, abortSignal = undefined) {
  return Object.freeze({
    kind,
    origin: blockOrigin(origin, blockOrdinal),
    inlines: parseInlineContent(text, origin, blockOrdinal, abortSignal),
  });
}

function parseInlineContent(text, origin, blockOrdinal, abortSignal = undefined) {
  const inlines = [];
  let cursor = 0;
...
function readListMarker(line) {
  ...
  if (["-", "*", "+", "–", "—", "•"].includes(line[0]) && line[1] === " ") {
    return { ordered: false, text: line.slice(2), indent, start: 1, marker: line[0] };
  }
  let index = 0;
  ...
  return { ordered: true, text: line.slice(index + 2), indent, start, marker: "." };
}

function isAnnouncedList(block) {
  if (block?.kind !== DOCUMENT_BLOCK_KIND.PARAGRAPH) return false;
  const text = block.inlines.map((inline) => inline.kind === "text" ? inline.text : inline.label).join("").trimEnd();
  return !text.trimStart().startsWith(">") && text.endsWith(":");
}
