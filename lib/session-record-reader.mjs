...
import { createRolloutReadStream } from "./rollout-source.mjs";
...
async function* streamProjectedRecords(file, summary, options) {
  ...
  const input = options.io?.createReadStream
    ? options.io.createReadStream(file, { highWaterMark: options.inputChunkBytes ?? 64 * 1024 })
    : createRolloutReadStream(file, { highWaterMark: options.inputChunkBytes ?? 64 * 1024, signal: options.abortSignal });
...
async function* streamLegacyReferenceRecords(file, summary, options) {
  ...
  const input = options.io?.createReadStream ? options.io.createReadStream(file) : createRolloutReadStream(file, { signal: options.abortSignal });
