...
async function* streamProjectedRecords(file, summary, options) {
  ...
  async function* sourceChunks() {
    try {
      ...
      if (error instanceof SessionRecordReaderError) throw error;
  ...
  try {
    ...
    throw normalizeReaderError(error, summary.recordCount + 1);
  } finally {
...
async function* streamLegacyReferenceRecords(file, summary, options) {
  ...
  try {
    for await (const line of lines) {
      ...
      let item;
      try {
        item = await projectLegacyValue(parsed, [], options);
      } catch (error) {
    ...
    if (rawHashError) throw rawHashError;
    if (error instanceof SessionRecordReaderError) throw error;
    throw readerError("SESSION_SOURCE_ERROR", summary.recordCount + 1, "Failed while reading the session source", error);
  } finally {
    lines.close();
  ...
  async accept(token) {
    switch (token.type) {
      ...
      case "array_start":
        ...
        return null;
      case "string_start":
        this.activeString = new ProjectedString(token.path, this.options);
        return null;
      case "string_chunk":
