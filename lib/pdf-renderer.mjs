...
import { createAbortCheckpoint, throwIfAborted } from "./export-abort.mjs";
...
const FONT_FILES = Object.freeze({
  ...
  emoji: Object.freeze({ name: "NotoEmoji", file: "NotoEmoji-Variable.ttf", sha256: "de6c18832938afc99caf132b39d6a30a19bac7f2e812e28db2535b4608d27551" }),
});
...
export async function buildDeterministicPdf(options = {}) {
  throwIfAborted(options.abortSignal, "PDF rendering");
  ...
  throwIfAborted(options.abortSignal, "PDF font loading");
  const glyphCache = new Map();
  const checkpoint = createAbortCheckpoint(options.abortSignal, "PDF block processing", 16);
  let doc;
  const chunks = [];
  ...
  try {
    ...
    for (const message of messages) {
      ...
      for (const block of message.blocks) {
        await checkpoint();
        renderBlock(doc, block, header.origin.sessionId, fonts, glyphCache);
      }
    ...
    throwIfAborted(options.abortSignal, "PDF page finalization");
    addPageNumbers(doc, header.origin.sessionId, fonts, glyphCache);
    throwIfAborted(options.abortSignal, "PDF page finalization");
    doc.end();
    const result = await completed;
    throwIfAborted(options.abortSignal, "PDF finalization");
    return result === null ? null : validateCanonicalPdf(result, { abortSignal: options.abortSignal });
  } catch (error) {
    ...
    completed.catch(() => {});
    if (error instanceof PdfExportError || error?.code === "EXPORT_CANCELLED") throw error;
    throw new PdfExportError(error?.code || "PDF_RENDER_ERROR", `Failed to render PDF for session ${header.origin.sessionId}`, error);
  }
}

export async function validateCanonicalPdfFile(filePath, options = {}) {
  throwIfAborted(options.abortSignal, "PDF validation");
...
export function validateCanonicalPdf(buffer, options = {}) {
  throwIfAborted(options.abortSignal, "PDF validation");
...
function renderHeader(doc, header, fonts, glyphCache) {
  ...
  const metadata = [
    ...
    ["Started", header.metadata.startedAt], ["Updated", header.metadata.updatedAt], [header.metadata.modelLabel || "Model", header.metadata.model],
...
function renderBlock(doc, block, sessionId, fonts, glyphCache) {
  ...
  if (block.kind === DOCUMENT_BLOCK_KIND.LIST) {
    ...
    block.items.forEach((item) => {
      ...
      const prefix = (item.ordered ?? block.ordered) ? `${number}. ` : `${item.marker || "•"} `;
      const baseIndent = block.announced ? 32 : 14;
      renderInlines(doc, [{ kind: "text", text: prefix }, ...item.inlines], {
        font: "regular", fontSize: BODY_SIZE, color: "#202020", indent: baseIndent + (item.level || 0) * 18, hanging: 14, after: 3, sessionId,
      }, fonts, glyphCache);
    });
    doc.moveDown(0.2);
    return;
...
function renderTextRuns(doc, sourceRuns, options, fonts, glyphCache) {
  ...
  for (const source of sourceRuns) {
    ...
    if (text.length) endsWithLineBreak = text.endsWith("\n");
    const lines = text.split("\n");
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
      for (const segment of validateAndSegment(lines[lineIndex], defaultFont, options.sessionId, fonts, glyphCache)) {
        logicalLines.at(-1).push({ ...source, ...segment });
      }
      if (lineIndex < lines.length - 1) logicalLines.push([]);
    }
  }
  if (endsWithLineBreak && logicalLines.length > 1 && logicalLines.at(-1).length === 0) logicalLines.pop();
  ...
  try {
    for (const logicalLine of logicalLines) {
      const runs = logicalLine.length ? logicalLine : [{ text: " ", font: defaultFont }];
...
function validateAndSegment(text, primaryStyle, sessionId, fonts, glyphCache) {
  ...
  for (const { segment: grapheme } of GRAPHEME_SEGMENTER.segment(text)) {
    const invalid = Array.from(grapheme).map((character) => character.codePointAt(0)).find(isSurrogateCodePoint);
    if (invalid !== undefined) {
      throw new PdfExportError("PDF_INVALID_UNICODE", `PDF export for session ${sessionId} cannot represent invalid UTF-16 unit U+${formatCodePoint(invalid)}`);
    }
    const font = selectFont(primaryStyle, grapheme, fonts, glyphCache);
    const rendered = font ? grapheme : unsupportedGlyphLabel(grapheme);
    const renderedFont = font || primaryStyle;
    if (renderedFont !== currentFont && currentText) {
      segments.push({ text: currentText, font: currentFont });
      currentText = "";
    }
    currentFont = renderedFont;
...
function selectFont(primaryStyle, grapheme, fonts, glyphCache) {
  if (grapheme === "\n" || grapheme === "\r") return primaryStyle;
  const key = `${primaryStyle}:${grapheme}`;
  if (glyphCache.has(key)) return glyphCache.get(key);
  let selected = "";
  const emojiPresentation = Array.from(grapheme, (character) => character.codePointAt(0)).includes(0xfe0f);
  if (!emojiPresentation && fontSupportsGrapheme(fonts[primaryStyle].parsed, grapheme)) selected = primaryStyle;
  else {
    const fallbacks = emojiPresentation ? ["emoji", "symbols", "symbols2", "mono"] : ["symbols", "symbols2", "emoji", "mono"];
    for (const fallback of fallbacks) {
      if (fallback !== primaryStyle && fontSupportsGrapheme(fonts[fallback].parsed, grapheme)) {
        selected = fallback;
        break;
      }
    }
...
function fontSupportsGrapheme(font, grapheme) {
  const codePoints = Array.from(grapheme, (character) => character.codePointAt(0));
  const required = codePoints.filter((codePoint) => !isGraphemeFormatCodePoint(codePoint));
  const visible = required.length ? required : codePoints;
  if (!visible.every((codePoint) => font.hasGlyphForCodePoint(codePoint))) return false;
  if (codePoints.length === 1) return true;
  const shaped = font.layout(grapheme);
  if (!shaped.glyphs.length || shaped.glyphs.some((glyph) => glyph.id === 0)) return false;
  const mappedCodePoints = shaped.glyphs.flatMap((glyph) => glyph.codePoints || []);
  return mappedCodePoints.length === codePoints.length
    && mappedCodePoints.every((codePoint, index) => codePoint === codePoints[index]);
}

function unsupportedGlyphLabel(grapheme) {
  const codePoints = Array.from(grapheme, (character) => character.codePointAt(0));
  if (codePoints.length === 1) return `[unsupported glyph U+${formatCodePoint(codePoints[0])}]`;
  return `[unsupported glyph ${codePoints.map((codePoint) => `U+${formatCodePoint(codePoint)}`).join(" ")}]`;
}

function isGraphemeFormatCodePoint(codePoint) {
