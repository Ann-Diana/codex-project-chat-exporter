...
const FONT_FILES = Object.freeze({
  ...
  emoji: Object.freeze({ name: "NotoEmoji", file: "NotoEmoji-Regular.ttf", sha256: "415dc6290378574135b64c808dc640c1df7531973290c4970c51fdeb849cb0c5" }),
});
...
function fontSupportsGrapheme(font, grapheme) {
  ...
  return visible.every((codePoint) => font.hasGlyphForCodePoint(codePoint));
}

function unsupportedGlyphLabel(grapheme) {
  const codePoints = Array.from(grapheme, (character) => character.codePointAt(0));
  if (codePoints.length === 1) return `[unsupported glyph U+${formatCodePoint(codePoints[0])}]`;
  return `[unsupported glyph sequence ${codePoints.map((codePoint) => `U+${formatCodePoint(codePoint)}`).join(" ")}]`;
}
