...
const KNOWN_TEXT_CONTENT_TYPES = new Set(["input_text", "output_text", "text"]);
const ATTACHMENT_CONTENT_TYPES = new Set(["input_image", "image_url", "local_image", "input_file", "file", "attachment"]);
const KNOWN_RAW_ONLY_CONTENT_TYPES = new Set(["refusal", "input_audio", "output_audio"]);

export const MESSAGE_CONTENT_KIND = Object.freeze({
  TEXT: "TEXT",
  ATTACHMENT: "ATTACHMENT",
  KNOWN_CONTENT_RAW_ONLY: "KNOWN_CONTENT_RAW_ONLY",
  UNKNOWN_RAW_ONLY: "UNKNOWN_RAW_ONLY",
  SCHEMA_INVALID_RAW_ONLY: "SCHEMA_INVALID_RAW_ONLY",
});

function jsonTypeLabel(value) {
  if (Array.isArray(value)) return "ARRAY";
  if (value === null) return "NULL";
  return typeof value === "undefined" ? "MISSING" : typeof value === "object" ? "OBJECT" : typeof value === "boolean" ? "BOOLEAN" : typeof value === "number" ? "NUMBER" : typeof value === "string" ? "STRING" : "NON_JSON";
}

export function classifyMessageContentPart(part) {
  if (typeof part === "string") return { kind: MESSAGE_CONTENT_KIND.TEXT, contentType: "", text: part, textField: "SELF" };
  if (Array.isArray(part)) return { kind: MESSAGE_CONTENT_KIND.UNKNOWN_RAW_ONLY, contentType: "" };
  if (!part || typeof part !== "object") return { kind: MESSAGE_CONTENT_KIND.SCHEMA_INVALID_RAW_ONLY, contentType: "" };

  const contentType = typeof part.type === "string" ? part.type : "";
  if (KNOWN_TEXT_CONTENT_TYPES.has(contentType)) {
    if (typeof part.text !== "string") {
      return { kind: MESSAGE_CONTENT_KIND.SCHEMA_INVALID_RAW_ONLY, actualType: jsonTypeLabel(part.text), contentType, knownText: true };
    }
    return { kind: MESSAGE_CONTENT_KIND.TEXT, contentType, text: part.text, textField: "text" };
  }
  if (!contentType) {
    for (const field of ["text", "input_text", "output_text"]) {
      if (typeof part[field] === "string") return { kind: MESSAGE_CONTENT_KIND.TEXT, contentType: "", text: part[field], textField: field };
    }
    return { kind: MESSAGE_CONTENT_KIND.UNKNOWN_RAW_ONLY, contentType: "" };
  }
  if (ATTACHMENT_CONTENT_TYPES.has(contentType)) return { kind: MESSAGE_CONTENT_KIND.ATTACHMENT, contentType };
  if (KNOWN_RAW_ONLY_CONTENT_TYPES.has(contentType)) return { kind: MESSAGE_CONTENT_KIND.KNOWN_CONTENT_RAW_ONLY, contentType };
  return { kind: MESSAGE_CONTENT_KIND.UNKNOWN_RAW_ONLY, contentType };
}

export function strictMessagePartText(part) {
  const classified = classifyMessageContentPart(part);
  return classified.kind === MESSAGE_CONTENT_KIND.TEXT ? classified.text : "";
}
...
export function extractReadingText(content, isRenderedImage = () => false) {
  if (!Array.isArray(content)) return strictMessagePartText(content);
  const omitted = new Set();
  let fence = null;
  for (let i = 0; i < content.length; i++) {
    const value = strictMessagePartText(content[i]);
    if (!fence && content[i]?.type === "input_text" && isOpeningImageMarker(value)
      && content[i + 1]?.type === "input_image" && isRenderedImage(content[i + 1])) {
      omitted.add(i);
      if (content[i + 2]?.type === "input_text" && strictMessagePartText(content[i + 2]) === "</image>") omitted.add(i + 2);
    }
  ...
  return content.filter((_, i) => !omitted.has(i)).map(strictMessagePartText).filter(Boolean).join("\n\n");
}
